...
class LLaDA(TemplateLM):
    ...
    def _create_model(
        ...
        **kwargs,
        ...
        if model_kwargs.get("peftmodel_path", None) is not None:
            self._model = PeftModel.from_pretrained(self._model, model_kwargs.get("peftmodel_path", None), torch_dtype=torch.bfloat16)
            print("Get The Perft Checkpoint From",{model_kwargs.get("peftmodel_path", None)})
        ...
        for batch in ds.iter(self.batch_size):
            ...
            for i in range(len(contexts)):
                ...
                if "Question:" in context and "Answer:" in context:
                    ...
                    for part in parts[1:-1]:  # 跳过第一个空部分和最后一个部分
                        if "Answer:" in part:
                            # 提取问题和答案
                            qa_parts = part.split("Answer:", 1)
                            if len(qa_parts) == 2:
                                question = "Question:" + qa_parts[0].strip()
                                answer = "Answer:" + qa_parts[1].strip()
                                fewshot = question + "\n" + answer
                                # 直接转换为token并添加到fewshots
                                fewshot_tokens =  self.tok_batch_encode(fewshot.strip(),truncation=self.truncation)[0][0]
                                fewshots.append(fewshot_tokens)
                    # 最后一个部分是我们问的问题，需要放到batch_question
                    question = "Question: "+parts[-1].strip()
                    question =  self.tok_batch_encode(question,truncation=self.truncation)[0][0]
                    batch_question_id.append(question)
                batch_fewshots_id.append(fewshots)
